		...
		while (true) {
			...
			if (isWithdraw.equals("네") || isWithdraw.equals("yes") || isWithdraw.equals("y")) {
				...
				if(projectService.updateMember(myAccount)) {
					System.out.println("정삭적으로 탈퇴되었습니다.");
					new LoginPage().start();
				}else{
					System.out.println("정삭적으로 처리되지 않았습니다.");
					return;
				};
			...
			if (!newPass.isEmpty()) {
				try {
					...
					if (projectService.updateMember(myAccount)) {
						...
						System.out.println("비밀번호를 수정할 수 없습니다.");
						return;
					}
				}
	...
	private void modifyBirth() {
		while (true) {
			...
			if (newBirth.length() == 6) {
				...
				System.out.println("다시 입력해주세요. ( 예시 : 19950101 or 950101");
				continue;
			}
		}
	}
